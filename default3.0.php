...
$sql="Select * from ".$list."  where gro=".$gr." and id>".$idlo." and id<".$idup;	
...
{
	$.ajax({url: "Up.php?id=<? echo $row["id"]; ?>" ,success: function (){$("#wdtb<? echo $row['id']; ?>").remove();}});
}
function postd<? echo $row["id"]; ?>()
{
	$.ajax({url: "Down.php?id=<? echo $row["id"]; ?>" ,success: function (){$("#wdtb<? echo $row['id']; ?>").remove();;}});
}
//function delrun()
//{
